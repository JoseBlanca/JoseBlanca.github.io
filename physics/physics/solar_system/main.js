...
function drawHoverDecorations() {
  function drawFor(b, isSelected) {
    const [sx, sy] = worldToScreen(b.x, b.y);
    ctx.strokeStyle = isSelected ? "rgba(255,210,90,0.95)" : "rgba(255,255,255,0.85)";
    ctx.lineWidth = isSelected ? 2 : 1.5;
    ctx.beginPath();
    ctx.arc(sx, sy, b.displayRadius + (isSelected ? 7 : 5), 0, Math.PI * 2);
    ctx.stroke();
    ...
    if (speed > 0) {
      ...
      const ux = b.vx / speed;
      const uy = b.vy / speed;
      drawArrow(sx, sy, sx + ux * len, sy + uy * len, "#7fe1a8");
    }
  }

  if (selectedBody) drawFor(selectedBody, true);
  if (hoveredBody && hoveredBody !== selectedBody) drawFor(hoveredBody, false);
}
...
function updateTooltip() {
  // Hover takes priority while the cursor is on a planet; otherwise show the
  // pinned (selected) planet's info, anchored next to the planet itself.
  const b = hoveredBody || selectedBody;
  if (!b) {
  ...
  const speed = b.speed();
  const sun = bodies[0];
  const distFromSun = Math.hypot(b.x - sun.x, b.y - sun.y);
  const pinned = !hoveredBody && selectedBody === b;

  tooltipEl.innerHTML = `
    <div class="name" style="color:${b.color}">${b.name}${pinned ? ' <span class="pin">●</span>' : ''}</div>
  ...
  // Anchor the tooltip: at the cursor while hovering, beside the planet when
  // only pinned (so it follows the body around the orbit).
  const pad = 14;
  let tx, ty;
  if (hoveredBody) {
    tx = mouseScreen.x + pad;
    ty = mouseScreen.y + pad;
  } else {
    const [px, py] = worldToScreen(b.x, b.y);
    tx = px + b.displayRadius + 10;
    ty = py + b.displayRadius + 10;
  }
  const tw = tooltipEl.offsetWidth;
  const th = tooltipEl.offsetHeight;
  if (tx + tw > canvasWidth - 4)  tx = (hoveredBody ? mouseScreen.x : worldToScreen(b.x, b.y)[0]) - tw - pad;
  if (ty + th > canvasHeight - 4) ty = (hoveredBody ? mouseScreen.y : worldToScreen(b.x, b.y)[1]) - th - pad;
  if (tx < 4) tx = 4;
  if (ty < 4) ty = 4;
...
let dragLast = null;
let dragMoveDistance = 0;          // accumulated movement during a press; if small, treat mouseup as a click
...
// A click on the canvas: select / deselect a planet. The browser fires this
// after mouseup; we use dragMoveDistance to skip clicks that were really pans.
canvas.addEventListener("click", (e) => {
  if (dragMoveDistance > 4) return;
  const rect = canvas.getBoundingClientRect();
...
function handleCanvasClick(sx, sy) {
  const body = findHoveredBody(sx, sy);
  if (!body) {
    // Click on empty space dismisses the pinned tooltip.
    selectedBody = null;
    return;
  }
  if (selectedBody === body) {
    // Click the already-selected planet to close the widget.
    selectedBody = null;
    return;
  }
  selectedBody = body;
  // Sync the editor panel so its sliders/values reflect the selected planet.
  if (bodySelect.value !== body.name) {
    bodySelect.value = body.name;
    syncSlidersFromBody();
  }
}
...
const massMultEl   = document.getElementById("massMult");
...
function currentEditBody() {
  return bodies.find(x => x.name === bodySelect.value);
}

// Sync sliders to the *current* state of the selected body relative to its
// initial values. (Lets the user switch planets without losing state.)
function syncSlidersFromBody() {
  const b = currentEditBody();
  if (!b) return;

  const massMult = b.initial.mass > 0 ? b.mass / b.initial.mass : 1;
  // mass slider is log10 of multiplier
  const exp = Math.max(-2, Math.min(2, Math.log10(Math.max(1e-9, massMult))));
  massSlider.value = exp.toFixed(2);

  const baseSpeed = Math.hypot(b.initial.vx, b.initial.vy);
  const speedMult = baseSpeed > 0 ? b.speed() / baseSpeed : 1;
  speedSlider2.value = Math.max(0, Math.min(2, speedMult)).toFixed(3);
...
function refreshEditDisplay() {
  const b = currentEditBody();
  if (!b) return;
  massValueEl.textContent = b.mass.toExponential(3) + " kg";
  speedValueEl.textContent = (b.speed() / 1000).toFixed(3) + " km/s";

  const massMult = b.initial.mass > 0 ? b.mass / b.initial.mass : 1;
  massMultEl.textContent = massMult.toFixed(2) + "×";

  const baseSpeed = Math.hypot(b.initial.vx, b.initial.vy);
  const speedMult = baseSpeed > 0 ? b.speed() / baseSpeed : 0;
  speedMultEl.textContent = speedMult.toFixed(2) + "×";
}

function applyMassFromSlider() {
  const b = currentEditBody();
  if (!b) return;
  const exp = parseFloat(massSlider.value);
  const mult = Math.pow(10, exp);
  b.mass = b.initial.mass * mult;
  refreshEditDisplay();
}
...
function applySpeedFromSlider() {
  const b = currentEditBody();
  if (!b) return;
  const mult = parseFloat(speedSlider2.value);
  const baseSpeed = Math.hypot(b.initial.vx, b.initial.vy);
  const newSpeed = baseSpeed * mult;
  const cur = b.speed();
  if (cur > 0) {
    ...
    // No current direction: use the initial direction as a fallback.
    const baseDirX = b.initial.vx / (baseSpeed || 1);
    const baseDirY = b.initial.vy / (baseSpeed || 1);
    b.vx = baseDirX * newSpeed;
    b.vy = baseDirY * newSpeed;
  }
  b.trail.length = 0;
  refreshEditDisplay();
}

bodySelect.addEventListener("change", syncSlidersFromBody);
massSlider.addEventListener("input", applyMassFromSlider);
speedSlider2.addEventListener("input", applySpeedFromSlider);
...
// Keep the display in sync after global resets, too.
document.getElementById("resetAll").addEventListener("click", syncSlidersFromBody);

syncSlidersFromBody();

// --- Panel open/close (mobile drawer) ----------------------------------
